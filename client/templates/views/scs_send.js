...
Template['views_scs_send'].helpers({
    /**
...
// Set basic variables
Template['views_scs_send'].onCreated(function () {
    var template = this;
	...
	var subChains = SubChains.find({}).fetch();
		subChain = subChains[0];
			...
			subChainVia = subChain.via,
			//from = subChain.address,
			owner = subChain.owner,
			monitorAddr = subChain.monitorAddr,
			monitorPort = subChain.monitorPort,
			dappAddr = subChain.dappAddr;
			scCoinUnit =  subChain.coinUnit;
	TemplateVar.set('subChainAddr',subChainAddr);
	TemplateVar.set('subChainVia',subChainVia);
	TemplateVar.set('monitorAddr',monitorAddr);
	TemplateVar.set('monitorPort',monitorPort);
	TemplateVar.set('dappAddr',dappAddr);
	TemplateVar.set('scCoinUnit',scCoinUnit);

	//console.log('scs :'+ selectedAccount );
	//Tracker.autorun(function(){
	//	Meteor.setInterval(function(){
	//	var subChainAddr = '0x8ae77a0549edbad27a6c7e03a91b7d382954abeb';//getFrom('.dapp-address-input .subChain-addr', 'value');
	//	var blockNumber = 0;
	//	//if( subChainAddr )
	//		//blockNumber = chain3.scs.getBlockNumber(TemplateVar.get("subChainAddr "));
	//		blockNumber = chain3.scs.getBlockNumber(subChainAddr );
	//	TemplateVar.set('blockNumber',blockNumber);
	//	console.log('setinterval'+blockNumber);
	//	},1000)
	//});
	...
	TemplateVar.set('isSending', false);
	TemplateVar.set('isDepositting', true);
    ...
    template.autorun(function (c) {
        //var address = TemplateVar.get('selectedAccount').address,
...
Template['views_scs_send'].onRendered(function () {
    var template = this;
...
Template['views_scs_send'].helpers({
    ...
    */
	...
	'selectedAccountList': function () {
        return [TemplateVar.get('selectedAccount')];
    },
	
	'subChainAddr': function(){
		return TemplateVar.get('subChainAddr');
	},
	
	'subChainVia': function(){
		return TemplateVar.get('subChainVia');
	},
	
	'monitorAddr': function(){
		return TemplateVar.get('monitorAddr');
	},
	
	'monitorPort': function(){
		return TemplateVar.get('monitorPort');
	},
	
	'dappAddr': function(){
		return TemplateVar.get('dappAddr');	
	},
    /**
...
//need to check subchain?
    ...
    'sendAllAmount': function () {
        ...
        if (TemplateVar.get('selectedToken') === 'mc') {
			...
			if(TemplateVar.get('selectedTransType') === 'isDepositting'){
				...
				amount = ScsAccounts.findOne({address: selectedAccount.address}).balance;
			}
    ...
    */
	...
	'selectedScsAccount': function(){
		var selectedAccount = Helpers.getAccountByAddress(TemplateVar.getFrom('.dapp-select-account.send-from', 'value'));
		return ScsAccounts.findOne({address: selectedAccount.address});

	},
	
	'selectScsAccounts': function(){
		...
		//var accounts = ScsAccounts.find({balance:{$ne:"0"}}, {sort: {balance: 1}}).fetch();
		var accounts = ScsAccounts.find({balance:{$ne:"0"}}, {sort: {balance: 1}}).fetch();
...
Template['views_scs_send'].events({
    /**
     Send subChainDapp
    ...
    */
    'submit form': function (e, template) {
		...
		if( TemplateVar.get("selectedTransType") === 'isSending' ){
			...
			to = TemplateVar.getFrom('.dapp-address-input .peer', 'value');
			TemplateVar.set("to",to);
		}else if( TemplateVar.get("selectedTransType") === 'isSending' ){
			
			to = selectedAccount.address;
		//}else{
		//	//alert("submit isWithdrawing 1");
        ...
        if (selectedAccount && !TemplateVar.get('sending')) {
			...
			{       
			  ...
			  chain3.mc.sendTransaction(
					...
					if(!err){
						...
						addScsTransactionAfterSend(txHash,amount,baseaddr,receive,0,0,receive,true,null,'0x02');
						FlowRouter.go('scs_dashboard');
					}else{
			...
			var depositToAppChain = function(baseaddr,basepsd, appchainaddr, amount)
			{
				...
				chain3.mc.sendTransaction( { from: baseaddr, 
				  ...
				  data: '0x6bbded70'},function(err,txHash){
					...
					if(!err){
						//alert('deposit 3');
						addScsTransactionAfterSend(txHash,amount,baseaddr,null,200000,chain3.mc.gasPrice,'0x6dded70',true,null,'0xfe');
						//alert('deposit 3a');
						FlowRouter.go('scs_dashboard');
					}else{
					  ...
					  { 
						...
						via: vnodevia
						  ...
						  if(!err){
								...
								addScsTransactionAfterSend(txHash,amount,null,baseaddr,0,0,dappbaseaddr + '89739c5b',true,null,'0x01');
								FlowRouter.go('scs_dashboard');
							}else{
			...
			if( TemplateVar.get("selectedTransType") === 'isDepositting' ){
				...
				McElements.Modal.question({
                    template: 'views_modals_scsSendTransactionInfo',
                    data: {
