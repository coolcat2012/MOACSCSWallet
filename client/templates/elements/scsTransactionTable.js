...
import './scsTransactionTable.html';
...
Template['elements_scs_transactions_table'].onCreated(function(){
    this._properties = {
        cursor: {}
    };
...
Template['elements_scs_transactions_table'].helpers({
    /**
    ...
    */
    'items': function(){
        var template = Template.instance(),
            ...
            collection = window[this.collection] || ScsTransactions,
...
Template['elements_scs_transactions_table'].events({
...
Template['elements_scs_transactions_row'].helpers({
    /**
    ...
    */
    'transactionType': function(){
		...
		if( this.isMicroChain && this.shardingFlag ){
			if(this.shardingFlag === '0x01'){
				return TAPi18n.__('wallet.scs.asm.send.isWithdrawing');
			}else if(this.shardingFlag === '0x02'){
				return TAPi18n.__('wallet.scs.asm.send.isSending');
			}else if(this.shardingFlag === '0xfe'){
				return TAPi18n.__('wallet.scs.asm.send.isDepositting');
			}
		}
...
Template['elements_scs_transactions_row'].events({
    /**
